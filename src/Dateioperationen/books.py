...
def main():
    ...
    with open('src/14/publishers.txt') as file:
        raw_data = file.readlines()
        publisher_names = [row[:-1] if row.endswith('\n') else row for row in raw_data if row.strip() != '']
    publisher1 = publishing_service.new_publisher(publisher_names[0])
    publisher2 = publishing_service.new_publisher(publisher_names[1])
